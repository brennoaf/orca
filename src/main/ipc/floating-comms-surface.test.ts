...
describe('floating communications IPC', () => {
  ...
  it('admits strict local open and update requests only', () => {
    ...
    expect(() =>
      handler('floatingComms:open')({ sender }, { ...openRequest, height: 419 })
    ).toThrow('floating_comms_open_denied')
    expect(() =>
      handler('floatingComms:update')({ sender }, { ...updateRequest, height: 419 })
    ).toThrow('floating_comms_update_denied')
  ...
  it('requires exact attached identity for measure and auxiliary close', () => {
    ...
    const measure = { ...identity, height: 520 }
    handler('floatingComms:measure')({ sender }, measure)
    handler('floatingComms:closeAttached')({ sender }, identity)
    expect(mocks.controller.measure).toHaveBeenCalledWith(sender, measure, 520)
    ...
    mocks.controller.isAttachedSender.mockReturnValue(true)
    expect(() => handler('floatingComms:measure')({ sender }, { ...measure, height: 419 })).toThrow(
      'floating_comms_measure_denied'
    )
    expect(() => handler('floatingComms:resize')({ sender }, { ...measure, height: 419 })).toThrow(
      'floating_comms_resize_denied'
    )
    mocks.controller.isAttachedSender.mockReturnValue(false)
    expect(() =>
  ...
  it('validates handoff shape and app pairing before detach', () => {
    ...
    const dockSnapshot = {
      ...
      revision: 3,
      visible: true,
      sessions: { discord: { appId: 'discord' as const } },
      layout: {
        version: 1 as const,
    ...
    mocks.communicationsDockController.openOrFocus.mockReturnValue(dockSnapshot)
    expect(handler('floatingComms:detach')({ sender }, detach)).toEqual(dockSnapshot)
  ...
  it('keeps disable and presentation reads local to the main renderer', () => {
