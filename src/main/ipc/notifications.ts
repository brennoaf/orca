...
>()
let dispatchRegisteredNotification:
  | ((
      args: NotificationDispatchRequest
    ) => NotificationDispatchResult | Promise<NotificationDispatchResult>)
  | null = null

export function dispatchMainNotification(
  args: NotificationDispatchRequest
): NotificationDispatchResult | Promise<NotificationDispatchResult> {
  if (!dispatchRegisteredNotification) {
    return { delivered: false, reason: 'disabled' }
  }
  return dispatchRegisteredNotification(args)
}
...
export function registerNotificationHandlers(store: Store, runtime?: OrcaRuntimeService): void {
  ...
  const dispatch = (
    ...
    if (runtime && args.source !== 'test' && args.source !== 'communication-message') {
  ...
  dispatchRegisteredNotification = dispatch
