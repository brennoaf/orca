...
vi.mock('../sandbox-preload-path', async () =>
  (await import('./createMainWindow-test-harness')).sandboxPreloadPathMock()
)
vi.mock('../native-appearance-windows', () => ({ registerNativeAppearanceWindow: vi.fn() }))
vi.mock('../messaging/discord-voice-app-lifecycle', () => ({
  startDiscordVoiceAppLifecycle: vi.fn()
}))
