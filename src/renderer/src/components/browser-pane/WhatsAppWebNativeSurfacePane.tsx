...
export function usesWhatsAppWebNativeSurface(
  ...
  return browserTab.floatingWorkspaceAppId === 'whatsapp-web'
}

function expectedOwnershipRejection(error: unknown): boolean {
  const message = error instanceof Error ? error.message : String(error)
  return (
    message.includes('whatsapp_fast_response_stale') ||
    message.includes('whatsapp_fast_response_sender_denied') ||
    message.includes('whatsapp_fast_response_browser_registration_denied')
  )
}

function hideStaleBrowserSurface(identity: BrowserSurfaceIdentity): void {
  void window.api.whatsappFastResponse.hide(identity).catch((error: unknown) => {
    if (!expectedOwnershipRejection(error)) {
      console.error('[whatsapp-fast-response] stale browser surface cleanup failed:', error)
    }
  })
}

function unregisterBrowserSurface(identity: BrowserSurfaceIdentity): Promise<void> {
  return window.api.whatsappFastResponse
    .unregisterBrowserSurface(identity)
    .catch((error: unknown) => {
      if (!expectedOwnershipRejection(error)) {
        console.error('[whatsapp-fast-response] browser surface unregister failed:', error)
      }
    })
}

export function WhatsAppWebNativeSurfacePane({
  browserTab,
  browserPage,
  isActive,
  inputLocked
  ...
  useLayoutEffect(() => {
    ...
    void enqueueBrowserSurfaceOperation(browserTab.id, async () => {
      ...
      const { registrationToken } =
        await window.api.whatsappFastResponse.registerBrowserSurface(registration)
      if (disposed) {
      ...
      {error ? (
        ...
        >
          <p className="text-sm text-muted-foreground">
            {translate(
              'communicationRail.whatsappWeb.browserError',
          ...
          >
            {translate('communicationRail.whatsappWeb.retry', 'Retry')}
          </Button>
        </div>
      ) : null}
    </div>
