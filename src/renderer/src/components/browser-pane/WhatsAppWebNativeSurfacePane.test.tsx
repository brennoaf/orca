...
describe('WhatsAppWebNativeSurfacePane', () => {
  ...
  beforeEach(() => {
    ...
    Object.assign(window, {
      api: { ui: { getZoomLevel: vi.fn(() => 0) }, whatsappFastResponse: api }
    })
    vi.stubGlobal(
      'ResizeObserver',
      class {
  ...
  it('selects the native surface only for the WhatsApp workspace app', () => {
    expect(usesWhatsAppWebNativeSurface({ floatingWorkspaceAppId: 'whatsapp-web' })).toBe(true)
    expect(usesWhatsAppWebNativeSurface({ floatingWorkspaceAppId: 'slack' })).toBe(false)
    expect(usesWhatsAppWebNativeSurface({})).toBe(false)
  })

  it('registers, attaches and unregisters the full WhatsApp surface without a webview', async () => {
  ...
  it('shows an actionable retry when registration fails', async () => {
    ...
    expect(screen.getByRole('alert').textContent).toContain('Could not open WhatsApp Web.')
